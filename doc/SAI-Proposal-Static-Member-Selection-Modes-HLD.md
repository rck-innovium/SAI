...
These modes are static in the sense that they do NOT adapt to ECMP/LAG member state.
...
1) Enum defining the static member selection mode :
   ...
   */
   ...
   {
      /** Static member selection mode based on Hash of packet fields Hash */
...
sai_create_lag_fn(
   ...
   sai_attr_list);
```
### Create ACL table and ACL entry
   ```c
   count = 0;
   ...
   sai_action_attr_list[count2++] = SAI_ACL_ACTION_TYPE_SET_ECMP_STATIC_MEMBER_SELECTION_MODE;
   sai_action_attr_list.list = action_attr_list;
   sai_action_attr_list.count = count2;
