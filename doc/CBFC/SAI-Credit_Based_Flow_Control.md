...
CBFC is a link-layer mechanism that ensures **lossless frame delivery** by _preventing frame loss due to buffer congestion at the receiver_. 
...
In CBFC, the sender ensures that the receiver has enough buffer before transmitting a frame. The receiver advertises its buffer availability to the sender using credits. These credits represent the amount of buffer space available. The sender can only transmit frames if it has sufficient credits—ensuring that the receiver has space to accept the incoming data. 
...
#### Virtual Channel (VC)
 ...
 Each VC can be configured as either lossless or best-effort, and this configuration can be changed dynamically. CBFC mechanism is applied on lossless VCs.
...
- New Qosmap types are added for VC derivation from TC, Queue or packet’s dot1p+DEI.
...
#### Figure 1: Overview of SAI objects for CBFC
![CBFC Overview](images/CBFC_Object_Overview.png "Figure 1: CBFC Overview")
...
#### Figure 2: Sender-side lossless operation (roles of Port/VC/Credit Profile/Credit Pool)
![CBFC Sender Overview](images/Objects_at_sender.png "Figure 2: CBFC Sender Overview")
...
#### Figure 3: Receiver-side lossless operation (roles of Port/VC)
![CBFC Receiver Overview](images/Objects_at_receiver.png "Figure 3: CBFC Receiver Overview")
...
- CBFC Virtual Channel is a per port object to enable/disable CBFC per VC, configure the credit management using credit profiles and provide CBFC stats. 
...
- Three new QoS map types added to sai_qos_map_type_t in saiqosmap.h
...
The existing QoS map params struct sai_qos_map_params_t in saitypes.h is extended to have the below fields to support the above new qosmap types.
...
3) [Sender] Set packet overhead (must be >= AI_PORT_ATTR_CBFC_RECEIVER_NATIVE_PACKET_OVERHEAD of partner).
...
5) [Sender] Set port sender credit limit

    Note: This step is applicable when the receiver lets the sender manage the port's credits among its VCs.

```c
/* SAI_PORT_ATTR_CBFC_SENDER_CREDIT_LIMIT must be <= the value got from the peer's
...
6) [Sender and Receiver] Create VC(s) and enable CBFC 
...
7) [Sender] Set QoS maps: dot1p/dei→TC, TC-to-Queue (existing), Queue-to-VC

```c
// {dot1p+DEI} → TC map (e.g. PCP=1,DEI=0 → TC2; PCP=2,DEI=0 → TC3)
sai_qos_map_t dot1p_dei_to_tc_entries[2] = {
    { .key = { .dot1p_dei = (1u << 1) | (0u << 0) }, .value = { .tc = 2 } },
    { .key = { .dot1p_dei = (2u << 1) | (0u << 0) }, .value = { .tc = 3 } },
};
...
8) [Receiver] Set QoS maps: dot1p/dei→TC, TC-to-VC 
...
// Apply TC→VC and dot1p/DEI→TC to ingress port (dot1p_to_tc_map_id from step 7 or create similarly)
...
2) [Sender] Stop dequeue on queue
...
3) [Sender] Attach credit profile to VC
...
- Ultra Ethernet Specification, **Chapter 5.2** — Link: `https://ultraethernet.org/wp-content/uploads/sites/20/2025/06/UE-Specification-6.11.25.pdf`
